...
## Rename files & Builda clinical data  frame
...
- Step1 : Read Downloaded clinical csv in both groups
    ...
    - 1. Read files
     ...
     ```{r}
     coldata_lobular <- read.csv("clinical.cart.2021-12-02.Lobu/clinicalLobular.tsv",sep = "\t")
     coldata_Duct <- read.csv("clinical.cart.2021-12-02Duct/clinicalDuct.tsv",sep = "\t")
     ```
...
- Step2 : Find the corresponding  TCGA id by filename and change htseq counts filenames 

     - 1. Map Filenames to TCGA id & Save it as `Duct/Lobular_filename_TCGAid.txt.txt`
     ...
     - 2. only chose the TCGA's first 12 characters in clinical file
     ...
     - 4.  Use `change_name.sh` to change the filenames into TCGA-Format
...
- Step3: Change filename 
     ...
     - 4. Change the filename (TCGA-ID format ) to "Ductgroup/Lobulargroup + Digital seria number" by their correspondence I build from Excel 
...
```
     ...
     ![2081638486290_ pic](https://user-images.githubusercontent.com/89502586/144517079-d170bf33-2ef0-4495-8861-f49e84336fef.jpg)
...
- Step5: Differential Expression Analysis 
     ...
     - 3. Partial Resutls of **res** ##Remove)
     ...
     - 4 . Partial Resutls of **resMF**



     ![2091638486773_ pic](https://user-images.githubusercontent.com/89502586/144517887-930f557e-7da4-4705-8312-d31c868c67bd.jpg)
...
- Step1:
     ...
     ```
     diff_gene <- subset(res, padj < 0.05 & abs(log2FoldChange) > 1.5)
     dim(diff_gene)
     head(diff_gene)
     ```
...
- Step2 : Name Transfer 
 ...
 ["symbolID_refiltered.csv"](https://github.com/Margery0011/510_Final_Project/blob/main/Results/symbolID_refiltered.csv)
