...
coldata_lobular <- read.csv("~/Scripts/clinicalLobular.tsv",sep = "\t")
```{r}
...
```{r}
metadata <- jsonlite::fromJSON("metadata.cart.duct12-01.json")
naid_df <- data.frame()
for (i in 1:nrow(metadata)){
  naid_df[i,1] <- metadata$file_name[i]
  naid_df[i,2] <- metadata$associated_entities[i][[1]]$entity_submitter_id
...
```{r}
library(readxl)
Lobular1<-read_excel("Lobulargroup_id.xlsx")
...
library(readxl)
Duct1<-read_excel("Ductgroup_id.xlsx")
